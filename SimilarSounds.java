...
class SimilarSounds
{
	...
	public static void populateWordToSoundMap(List<String> lines) {
		...
		for (String line: lines) {
			wordToSound.put(Extractor.extractWordFromLine(line), Extractor.extractSoundFromLine(line)); // hashmap.put(word, sound)
		}
	}
	...
	public static void populateSoundGroupToSimilarWordsMap(List<String> lines) {
		...
		for (String line: lines) {
			String key = Extractor.extractSoundGroupFromSound(Extractor.extractSoundFromLine(line)); // key = sound
			BST<String> value;
			
			// If the sound group has not been put in the hashmap before
			if (soundGroupToSimilarWords.get(key) == null) {
				value = new BST<>();
				value.insert(Extractor.extractWordFromLine(line));

				soundGroupToSimilarWords.put(key, value); // hashmap.put(sound, soundgroup)
			}

			// If the sound group already has been put in the hashmap
			else {
				soundGroupToSimilarWords.get(key).insert(Extractor.extractWordFromLine(line)); // Insert word into BST
			}
		}
	}
	...
	public static void findSimilarWordsInList(String words[]) {
		...
		ArrayList<String> usedWords = new ArrayList<>(); // List of already used words
		ArrayList<String> unrecognizedWords = new ArrayList<>(); // List of unrecognized words
		String string = ""; // Final string to print

		for (int i = 0; i < words.length; i++) { // For every word
			
			// Skip if not a recognized word
			if (wordToSound.get(words[i].toUpperCase()) == null) {
				unrecognizedWords.add(words[i]);
				continue;
			}

			// Skip if the word is a duplicate
			else if (usedWords.contains(words[i].toUpperCase())) {
				continue;
			}

			usedWords.add(words[i].toUpperCase());
			string += "\"" + words[i] + "\" sounds similar to:";

			// Lists all the words the word is similar to
			boolean hasSimilarWords = false; // used to put "none" if there are no similar words
			for (int l = i + 1; l < words.length; l++) {
				// Add the word if it is similar
				if (wordToSound.get(words[l].toUpperCase()) != null && !usedWords.contains(words[l].toUpperCase()) && Extractor.extractSoundGroupFromSound(wordToSound.get(words[i].toUpperCase())).equals(Extractor.extractSoundGroupFromSound(wordToSound.get(words[l].toUpperCase())))) {
					string += " \"" + words[l] + "\"";
					hasSimilarWords = true;
					usedWords.add(words[l].toUpperCase());
					System.out.println();
				}
			}
		...
		string += "Unrecognized words:";
		for (String word: unrecognizedWords) {
			string += " \"" + word + "\"";
		}
		if (unrecognizedWords.size() < 1) {
			string += " none";
		}
	...
	public static void findSimilarWordsTo(String theWord) {		
		...
		if (wordToSound.get(theWord.toUpperCase()) != null) {
			System.out.println("Words similar to \"" + theWord + "\": " + soundGroupToSimilarWords.get(Extractor.extractSoundGroupFromSound(wordToSound.get(theWord.toUpperCase()))));
		}
		else {
